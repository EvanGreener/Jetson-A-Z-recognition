...
while True:
    ...
    # Make sure --input-width --input-height are the SAME
    IMG_WIDTH = 28
    img = input.Capture(format='gray8')
    resized_img = cudaAllocMapped(
        width=IMG_WIDTH, height=IMG_WIDTH, format=img.format)
    cudaResize(img, resized_img)
    # classify the image
    class_id, confidence = net.Classify(resized_img)
    # find the object description
    class_desc = net.GetClassDesc(class_id)
    # overlay the result on the image
    font.OverlayText(resized_img, resized_img.width, resized_img.height, "{:05.2f}% {:s}".format(
        confidence * 100, class_desc), 5, 5, font.White, font.Gray40)
    # render the image
    output.Render(resized_img)
